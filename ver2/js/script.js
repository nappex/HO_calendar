var cal = {
  // (A) PROPERTIES
  monthNames : ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], // Month Names
  ...
  list : function () {
    ...
    var squares = [];
    if (cal.startMonday && startDay != 1) {
      var blanks = startDay==0 ? 7 : startDay ;
      for (var i=1; i<blanks; i++) { squares.push("b"); }
    }
    if (!cal.startMonday && startDay != 0) {
    ...
    if (cal.startMonday && endDay != 0) {
      var blanks = endDay==6 ? 1 : 7-endDay;
      for (var i=0; i<blanks; i++) { squares.push("b"); }
    }
    if (!cal.startMonday && endDay != 6) {
